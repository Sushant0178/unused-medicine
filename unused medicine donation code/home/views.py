...
def medicine_donation_form(request):
    if request.method == 'POST':
        form = MedicineForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('thank_you')  # Redirect to a thank you page or any other appropriate URL
    else:
        form = MedicineForm()

    return render(request, 'medicine_donation_form.html', {'form': form})
